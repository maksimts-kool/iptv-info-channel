...
export function wrapText(text, maxChars, maxLines) {
  ...
  if (lines.length <= maxLines) return lines.map((line) => clipText(line, maxChars));
  const kept = lines.slice(0, maxLines);
  kept[maxLines - 1] = clipText(`${kept[maxLines - 1]} ${lines[maxLines]}`, maxChars);
  return kept;
}

function noticeWhen(notice) {
...
// Render the loop frames into dir. Returns { slide1, card, status? } paths; the
// global status slide is rendered only when its model is supplied.
export async function renderSlidesPng(
  user, settings, dir, plans = [], summary = null, subscribeUrl = null,
) {
  const out = {
