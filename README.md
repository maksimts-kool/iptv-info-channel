...
#    PUBLIC_BASE_URL must be the address your players use, e.g. http://192.168.1.50:8080
...
#    http://<host>:8080/admin   (log in with ADMIN_PASSWORD)
```
...
```
http://<host>:8080/u/<token>/playlist.m3u
```
...
`http://<host>:8080/admin` — sign in with `ADMIN_PASSWORD`.
...
|---|---|---|
| `PORT` | `8080` | HTTP port. |
| `PUBLIC_BASE_URL` | `http://localhost:8080` | Address embedded in m3u/HLS URLs. Set to host IP on a LAN. |
